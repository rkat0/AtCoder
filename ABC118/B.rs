...
fn main() {
    ...
    let n: usize = sc.next();
    let m: usize = sc.next();
    let v0: Vec<usize> = sc.next_vec_len();
    let mut set: HashSet<usize> = v0.iter().map(|x| *x).collect();
    for i in 1..n {
        let v: Vec<usize> = sc.next_vec_len();
        let set_t: HashSet<usize> = set.intersection(&v.iter().map(|x| *x).collect()).map(|x| *x).collect();
        set = set_t;
    }
    println!("{}", set.len());
}
